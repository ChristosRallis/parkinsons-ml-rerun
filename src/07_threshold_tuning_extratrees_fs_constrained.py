# 07_threshold_tuning_extratrees_fs_constrained.py
import warnings
...
PREDS_DIR = BASE / "results" / "evaluation_extratrees_fs" / "predictions"
OUT_DIR = BASE / "results" / "evaluation_extratrees_fs" / "threshold_tuning"
OUT_DIR.mkdir(parents=True, exist_ok=True)

THRESHOLDS = np.arange(0.01, 1.00, 0.01)
MIN_RECALL_CONSTRAINT = 0.90

print("Starting Constrained Threshold Tuning (Pipeline: ExtraTrees FS)...")
...
for p_file in pred_files:
    model_name = p_file.stem.replace("(1)_predictions_", "")
    ...
    for th in THRESHOLDS:
        ...
        cand.append(m)
        detailed_rows.append({"Model": model_name, **m})

    cand_df = pd.DataFrame(cand)
    valid = cand_df[cand_df["Recall"] >= MIN_RECALL_CONSTRAINT]

    if not valid.empty:
        best = valid.sort_values(by=["F1", "Balanced_Acc"], ascending=[
                                 False, False]).iloc[0]
        note = "Constraint Met ✅"
    else:
        best = cand_df.sort_values(by=["F1", "Balanced_Acc"], ascending=[
                                   False, False]).iloc[0]
        note = f"Constraint Failed (Max Recall: {cand_df['Recall'].max():.2f}) ⚠️"

    print(
        f"   ✅ Best Th: {best['Threshold']:.2f} | F1: {best['F1']:.4f} | Recall: {best['Recall']:.4f} | {note}")
    ...
    summary_rows.append({
        ...
        "Best_Bal_Acc": round(float(best["Balanced_Acc"]), 4),
