# 05d_extratrees_fs_tuning_find_best_k_mlp.py
# ------------------------------------------------------------
# Tuning only ExtraTrees selector parameter (fs__max_features) for:
# StandardScaler -> SelectFromModel(ExtraTrees) -> MLP
#
...
OUT_DIR = BASE / "results" / "evaluation_extratrees_fs_mlp" / "extratrees_tuning_mlp"
...
def build_pipeline(best_mlp_params: dict):
    ...
    fs = SelectFromModel(
        estimator=et,
        threshold=-np.inf  # so selection is controlled by max_features (top-k)
    )
...
# LOAD DATA
# ==========================================
print("📥 Loading data/folds/best-params (ExtraTrees FS tuning MLP)...")
...
grid = GridSearchCV(
    ...
    scoring=SCORING,
    cv=custom_cv,
    n_jobs=1,  # keep stable on Windows
    refit=True,
